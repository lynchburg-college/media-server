			...
			return jq.each(function(){
				...
				trs.draggable({
					...
					proxy: function(source) {
						...
						var p = $('<table></table>').appendTo('body');
						tr2.clone().removeAttr('id').appendTo(p);
						tr1.clone().removeAttr('id').find('td').insertBefore(p.find('td:first'));
						$('<td><span class="tree-dnd-icon tree-dnd-no" style="position:static">&nbsp;</span></td>').insertBefore(p.find('td:first'));
						p.find('div').css('vertical-align','middle');
						p.hide();
						return p;
					},
					deltaX: 15,
					...
					onDrag: function(e) {
						...
						if (d>12){	// when drag a little distance, show the proxy object
